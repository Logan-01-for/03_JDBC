...
// (Model 중 하나)Service : 비즈니스 록직을 처리하는 계층,
...
public class UserService {
	...
	//메서드
	//		참조형
	/**전달 받은 아디디와 일치하는 User 정보 반환 서비스
	 * @param input (입력된 아이디)
	...
	/** USER_NO 를 입력 받아 일치하는 User 삭제 서비스
	 * @param input
	 * @return result 
	 */
	public int deleteUser(int input) throws Exception{
		...
		// 결과에 따른 트랜잭션 처리 필요 commit/ return
		if(result > 0) {
			JDBCTemplate.commit(conn);
	...
	public int multiInsertUser(List<User> userList) throws Exception{
		...
		// 2) Java 반복문을 이용한 다중 INSERT (이거 사용1)
		...
		if (count == userList.size()) {// 크기 size
			JDBCTemplate.commit(conn);
