...
public class UserDAO {
	...
	public List<User> selectAll(Connection conn) throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				// java.sql.Date타입으로 값을 저장하지 않는 이유
				// -> SELECT 문에서 TO_CHAT()를 이용하여 문자열로 변환해 조회 했기 때문에
	...
	/** USER_NO 를 입력 받아 일치하는 User 삭제 DAO
	 * @param conn
	 * @param input
	 * @return result
	 */
	...
	public int idCheck(Connection conn, String userId) throws Exception{
		...
		try {
			...
			if(rs.next()) {
				count = rs.getInt(1); // OR ("COUNT (*)") 조회된 컬럼 순서 번호를 이용해
												// 컬럼 값 얻어오기
			}
